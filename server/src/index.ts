...
interface User {
  ...
  currentPeerSocketId: string | null; // The socket ID of the person they are currently calling
}

// Store all connected users by their socket ID
const users: Map<string, User> = new Map();
// Store users waiting for a call
const callQueue: string[] = []; // Contains socket IDs
    ...
    if (callerSocketId && calleeSocketId) {
      ...
      if (caller && callee) {
        ...
        if (caller.status === 'available' && callee.status === 'available') {
          ...
          if (caller.status === 'available') callQueue.unshift(callerSocketId);
          if (callee.status === 'available') callQueue.unshift(calleeSocketId);
        ...
        if (callerSocketId) callQueue.unshift(callerSocketId); // Put back if exists
        if (calleeSocketId) callQueue.unshift(calleeSocketId); // Put back if exists
...
const updateUserListForAll = () => {
  const usersInfo = Array.from(users.values()).map(u => ({
    id: u.userId,
    status: u.status,
    peer: u.currentPeerSocketId ? users.get(u.currentPeerSocketId)?.userId : null
  }));
...
io.on('connection', (socket) => {
  ...
  socket.on('offer', (targetSocketId: string, offer: RTCSessionDescriptionInit) => {
    // console.log(`Offer from ${socket.id} to ${targetSocketId}`);
    io.to(targetSocketId).emit('offer', socket.id, offer);
  });

  socket.on('answer', (targetSocketId: string, answer: RTCSessionDescriptionInit) => {
    // console.log(`Answer from ${socket.id} to ${targetSocketId}`);
    io.to(targetSocketId).emit('answer', socket.id, answer);
  });

  socket.on('ice-candidate', (targetSocketId: string, candidate: RTCIceCandidate) => {
  ...
  socket.on('end-call', () => {
    ...
    if (currentUser && currentUser.status === 'in-call' && currentUser.currentPeerSocketId) {
      ...
      console.log(`${currentUser.userId} (${socket.id}) is ending call with ${peer?.userId} (${peerSocketId})`);
      ...
      attemptMatchmaking(); // Attempt to match the now-available users
        ...
        console.log(`${currentUser?.userId} (${socket.id}) tried to end a call but was not in one.`);
    }
  });
